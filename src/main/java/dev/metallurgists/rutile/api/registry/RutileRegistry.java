...
public abstract class RutileRegistry<K, V> implements Iterable<V> {
    public static final Map<ResourceLocation, RutileRegistry<?, ?>> REGISTERED = new HashMap<>();
    ...
    public RutileRegistry(ResourceLocation registryName) {
    ...
    public void freeze() {
        ...
        if (!checkActiveModContainerIsRutile()) {
            return;
        }
    ...
    public <T extends V> T replace(K key, T value) {
        ...
        if (!containKey(key)) {
            Rutile.LOGGER.warn("[replace] couldn't find key %s in registry %s".formatted(registryName, key));
        }
        registry.put(key, value);
        return value;
    }
    ...
    public static class String<V> extends RutileRegistry<java.lang.String, V> {
    ...
    public static class RL<V> extends RutileRegistry<ResourceLocation, V> {
